...
class FSRCNN(nn.Module):
    def __init__(self,format_type):
        super().__init__()
        self.feature_extraction=nn.Sequential(nn.Conv2d(channel[format_type],56,
                                                        kernel_size=5,
                                                        stride=1,
                                                        padding=2),
                                              nn.PReLU())
        self.shrinking=nn.Sequential(nn.Conv2d(56,12,
                                               kernel_size=1,
                                               stride=1,
                                               padding=0),
                                     nn.PReLU())
        self.nonlinear_mapping=nn.Sequential(nn.Conv2d(12,12,
                                             ...
                                             nn.Conv2d(12,12,
                                                      ...
                                                      padding=1),
                                             nn.PReLU())
        self.expanding=nn.Sequential(nn.Conv2d(12,56,
                                               kernel_size=1,
                                               stride=1,
                                               padding=0),
                                     nn.PReLU())
        self.subpixel=nn.Sequential(nn.Conv2d(56,channel[format_type]*upscale_factor**2,
                                              kernel_size=1,
                                              stride=1,
                                              padding=0),
                                    nn.PReLU(),
    ...
    def forward(self,x):
        ...
        shrinking=self.shrinking(feature_extraction)
        nolinear_mapping=self.nonlinear_mapping(shrinking)
        expanding=self.expanding(nolinear_mapping)
        subpixel=self.subpixel(expanding)
        return subpixel
...
class ESPCN(nn.Module):
    def __init__(self,format_type):
        ...
        self.nonlinear_mapping=nn.Sequential(nn.Conv2d(channel[format_type],64,
                                             ...
                                             nn.Conv2d(64,32,
                                                       ...
                                                       padding=1),
                                             nn.Tanh())
        self.subpixel=nn.Sequential(nn.Conv2d(32,channel[format_type]*upscale_factor**2,
                                              kernel_size=1,
                                              stride=1,
                                              padding=0),
                                    nn.PReLU(),
    ...
    def forward(self,x):
        ...
        subpixel=self.subpixel(nolinear_mapping)
        return subpixel
...
class SRResNet(nn.Module):
    def __init__(self,format_type):
        super().__init__()
        self.feature_extraction=nn.Sequential(nn.Conv2d(channel[format_type],64,
                                                        kernel_size=9,
                                                        stride=1,
                                                        padding=4),
                                              nn.PReLU())
        self.RB=nn.Sequential(utils.ResidualBlock(64,64),
                              ...
                              nn.Conv2d(64,64,
                                        ...
                                        stride=1,
                                        padding=1))
        self.subpixel=nn.Sequential(nn.Conv2d(64,64*upscale_factor**2,
                                              kernel_size=3,
                                              stride=1,
                                              padding=1),
                                    nn.PReLU(),
                                    nn.PixelShuffle(upscale_factor),
                                    nn.PReLU())
        self.construction=nn.Sequential(nn.Conv2d(64,channel[format_type],
                                                  kernel_size=9,
                                                  stride=1,
                                                  padding=4)
    ...
    def forward(self,x):
        feature_extraction=self.feature_extraction(x)
        RB=self.RB(feature_extraction)
        subpixel=self.subpixel(feature_extraction+RB)
        construction=self.construction(subpixel)
        return construction
